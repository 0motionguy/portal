...
```json
{
  ...
  "manifest": { /* ...validated v0.1.5 manifest... */ },
  "readiness_score": { /* ...AISO score v1, optional... */ },
  "last_checked": "2026-04-30T13:00:00Z"
}
```

`manifest` MUST validate against the v0.1.5 schema (use `validateManifest` from `@visitportal/spec`). `readiness_score` is the [AISO score envelope](./aiso-readiness-score.md) — optional; when absent, badge state is computed from `manifest` alone.

## Discovery

TrendingRepo polls the repo's homepage URL (resolved from the GitHub `homepage` field, falling back to README links). Order:

1. `<homepage>/.well-known/portal.json` (preferred, per [spec Appendix E](../spec-v0.1.5.md))
2. `<homepage>/portal`
...
- **1.0.0 (2026-04-30)** — initial 4-state badge + 3-line preview. Aligned to spec v0.1.5 and AISO readiness-score v1.0.0.
