...
**Status:** Stable · v1.2 (x402 v2 wire, production-verified) · Non-normative in base spec · Reference implementation: [`@visitportal/x402-adapter`](../packages/x402-adapter) (v0.1.11+) · Last updated: 2026-04-30

**Production verification:** end-to-end round-trip executed against the live [AISO Scan](https://aiso.tools) endpoint on Sponge gateway (`https://api.paysponge.com/x402/purchase/svc_d7nqes8p13x5p4888/api/scan`) using `@visitportal/x402-adapter@0.1.11` + the test-payer signing flow. Facilitator accepts the signed v2 `PaymentPayload`, validates the EIP-3009 `TransferWithAuthorization` against the USDC contract's EIP-712 domain, and only rejects on `insufficient_balance` when the wallet is unfunded. **Wire shape and signing logic are correct against production x402 v2 infrastructure.**
