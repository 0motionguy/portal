...
`portal.json` is served verbatim from `GET /portal`. Required fields: `portal_version`, `name`, `brief`, `tools`, `call_endpoint`. See [spec v0.1.0 §4](./spec-v0.1.0.md) for the full schema.
...
Application errors always return HTTP 200 with `{ ok: false, error, code }`. The `code` is one of `NOT_FOUND | INVALID_PARAMS | UNAUTHORIZED | RATE_LIMITED | INTERNAL` ([spec §6](./spec-v0.1.0.md#6-error-codes-normative)). Transport-level failures (4xx/5xx) are fine too — visitors surface them as `CallFailed`.
...
v0.1 is deliberately small. For verified-agent identity, per-call micropayments, stateful sessions, or registry discovery, see [`docs/spec-v0.1.0.md` §8](./spec-v0.1.0.md#8-optional-extensions-non-normative). Extensions (PE-001 ERC-8004, PE-002 x402, PE-003 AGP, PE-004 ClawPulse) layer on top of the base — a Portal that uses them MUST still be valid under v0.1.0.
