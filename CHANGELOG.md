...
- **Architecture overview doc.** [`docs/architecture-overview.md`](docs/architecture-overview.md) is the one-page assembly across nine sections (manifest schema, call API, server examples, security, AISO, TrendingRepo, testing, what-not-to-build).
- **Directory entries.** Static-fallback and CF Worker references now appear in `web/public/directory.json`.
